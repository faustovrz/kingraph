...
adj.from.phi <- function(phi, thresh = -200, negtozero = TRUE){
...
write.kinship.matrix <- function(adj, file="phi_matrix.tab"){
  write.table(adj$matrix,file=file, quote=FALSE, sep="\t")
}

adj.heatmap <- function(adj, pdf.base="phi.heatmap.pdf",
                        phi.thresh = "",symm=TRUE){
  if(is.matrix(adj)){
    matrix <- adj
  }else if(is.matrix(adj$matrix)){
    matrix <- adj$matrix
  }
  
    if(phi.thresh != ""){
    pdf.out <- paste(pdf.base,phi.thresh,"pdf",sep=".")
  }else{
    pdf.out <- paste(pdf.base,"pdf",sep=".")
  }
  
  dims <- ceiling(dim(matrix)/100)*10
  
  pdf(pdf.out,height=dims[1], width=dims[2])
  
  heatmap.2(matrix, col=phi.palette, breaks = phi.breaks,
...
vis.kingraph <- function(g,legend.arg=NULL,
                         bipartite = FALSE,
                         remove.phi=NULL){
  ...
  v <- visIgraph(g)  %>% visOptions(width = "1280px", height="960px",
                                    nodesIdSelection = TRUE,
                                    highlightNearest = TRUE)
  if(!is.list(legend.arg) &
     (is.character(V(g)$collection) |
      is.factor(V(g)$collection))){
...
kin.stepIVS <- function(phi, upper=0.45, lower=0.0442, nv = 10){
  ...
  step.ivs$phi <- phi[ !(phi$INDV1 %in% names(step.redundant)) &
                         !(phi$INDV2 %in% names(step.redundant)),]
  ...
  V(step.ivs$g)$color.border[V(step.ivs$g)$name %in%
                               names(step.ivs$independent)] <- "black"
  step.ivs
}
...
kin.bipartite.graph <-function(g, remove.unrelated=FALSE){
  #check bipartite
  if (!is.bipartite(g)){
    stop(paste("kinship graph is not bipartite. Check vertex info!", levels(type)))
  }
  
  part1 <- V(g)$name[which(V(g)$type == FALSE)]
  part2 <- V(g)$name[which(V(g)$type == TRUE)]
  bp.combo <- expand.grid(V1 = part1, V2 = part2)
  g.edges <- as.data.frame(get.edgelist(g))
  bp.edges <-c(t(merge(g.edges,
                       bp.combo,
                       by=c("V1","V2"))))
...
find.dup <- function(phi,thresh =0.45, vertex.info = NULL){
...
compare.bipart <- function(g, adj){
  if(is.bipartite(g)){
  is.part1 <- V(g)$type == FALSE
  is.part2 <- V(g)$type == TRUE
  
  return(list( adj = adj$matrix[names(V(g)[is.part1]),
                                names(V(g)[is.part2])],
                 g = remove.degree.0(kin.bipartite.graph(g))))
  } else{
    stop("kinship graph not bipartite. Add type to vertices (add.vertex.info)")
  }
}

kin.stats <-function(g){
  list(unrelated = sum(degree(g) == 0),
       related.fr = sum(degree(g) > 0) / vcount(g),
       connectivity = ecount(g) / (vcount(g)*(vcount(g) - 1) / 2))
}

kinship.analysis <- function(phi.file,vertex.info=data.frame(),ivs.thresh=0.177, results.dir="results"){
  ...
  if(length(vertex.info) > 0){
    if(length(levels(as.factor(V(dup$g)$type)))==2 ){
      col1 <- which(vertex.info$collection==levels(vertex.info$collection)[1])
      col2 <- which(vertex.info$collection==levels(vertex.info$collection)[2])
  ...
  if(length(vertex.info) > 0){
    kin.ivs$g <- add.vertex.info(kin.ivs$g, vertex.info)
